...
/** `RequiredKeys` is used to get all required fields, combined into a union type */
type RequiredKeys<T> = {
  [P in keyof T]: T extends Record<P, T[P]> ? P : never;
}[keyof T]

/** `OptionalKeys` is used to get all optional fields, combined into a union type */
type OptionalKeys<T> = {
  [P in keyof T]: object extends Pick<T, P> ? P : never;
}[keyof T]

/** `GetRequired` is used to get a new type consisting of all required keys and their types */
type GetRequired<T> = {
  [P in RequiredKeys<T>]-?: T[P];
}

/** `GetOptional` is used to get a new type consisting of all optional keys and their types */
type GetOptional<T> = {
